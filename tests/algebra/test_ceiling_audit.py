...
**ولا دَينَ أعمى**: صُرِّح بسقفِ **أربعةٍ وثلاثين** شرطًا من خمسةٍ
وثلاثين، **وواحدٌ مُصنَّفٌ بسببه** في `CEILING_NEEDS_DERIVATION` —
وسببُه أنّ إحصاءه **ثمنُ هروبٍ وهجاء**، **فسقفُه يحتاج
...
CEILING_NEEDS_DERIVATION: dict[str, tuple[tuple[str, str], ...]] = {
    "test_markov_ladder_run.py": (
        (
            "ي٥",
            "ثمنُه **هروبٌ ثمّ هجاء**: `(longest + 1) + width·log2(112)`، "
            "و`longest` أطولُ شفرةِ هفمان على المسند — **وليست في السجلّ**. "
            "فسقفُه يحتاج حدًّا على أطول شفرةٍ، ولم يُشتَقّ",
        ),
    ),
}
CLASSIFIED = 1
...
def test_the_guard_says_what_it_does_not_do() -> None:
    ...
    assert "**وواحدٌ مُصنَّفٌ بسببه**" in text
