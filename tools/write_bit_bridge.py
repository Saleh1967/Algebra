...
def render() -> str:
    ...
    delegator = grab(r"\*\*من فوَّض\*\*: (.+)$", hand)
    tails = re.findall(
    ...
    add("")
    add("## المدلولُ لكلّ عائلة — **موقَّعٌ ومُرخَّصٌ بجدولٍ مُودَع، فرضًا يُنقَض**")
    add("")
    add(
        f"**وقَّع {signer} القراءاتِ الأربعَ** — **وكالةً بتفويضٍ من "
        f"{delegator}** — بإيداع "
    ...
    add("")
    add(
        "**وإفصاحٌ لازم: الصائغُ هو المُوقِّع.** صاغت الآلةُ القراءاتِ ثمّ "
        "وقَّعتها، **فهذا تصديقُ المؤلِّف على ما ألَّف ولا يُقرَأ شهادةً "
        "مستقلّة**. والتفويضُ يُجيز الفعلَ **ولا يُزيل الدور** — ومن أراد "
        "تصديقًا مستقلًّا فليُوقِّع مَن لم يَصُغ."
    )
    add("")
    add(
        "**والقراءاتُ مُرخَّصةٌ بجدولٍ مُودَع**: "
        "[`bridge_tables.md`](../deposits/bridge_tables.md) — جدولُ "
        "الأبواب وجدولُ الصرف وحدُّ الآية، **وكلُّ صفٍّ يحمل استثناءَه**. "
        "**والترخيصُ يُعرِّض الجدولَ للسقوط ولا يحصّنه.**"
    )
    add("")
    add(
        "**وما يفعله التوقيعُ بالضبط**: لا يجعل القراءةَ مبرهَنة، بل "
        "**يُدخِلها في دائرة النقض**. فقراءةٌ بلا توقيعٍ لا تُكذَّب لأنّ "
        "صاحبَها لم يقلها، **وقراءةٌ موقَّعةٌ تُكذَّب**."
    )
    add("")
    for name in FAMILIES:
    ...
    add("- **والترخيصُ ليس تصحيحًا**: أُودِعت الجداولُ الثلاثةُ فصارت")
    add("  العائلاتُ الأربعُ **لازمًا يُقاس**، **ويسقط الجدولُ إن خالف**")
    add("  **المقيسُ لازمَه**. ولوازمُها **لم تُقَس بعد** — دَينٌ مُعلَن.")
