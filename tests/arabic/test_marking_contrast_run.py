...
**والجوابُ: لا.** وهذا الختمُ إنّما جاء ليصحّح عزلًا كان فيه متغيّران (العطل
١٣)، فسوّى **قيمةَ** العمق في الذراعين. **وتسويةُ القيمة ليست تسويةَ الأثر**:
وسمُ الأبديِّ يُخرِج المرفوضَ من الترتيب إخراجًا دائمًا، فتقلّ إعادةُ عرضِه،
بينما المؤقّتُ يعيده عند كلّ التزام. فالنافذةُ نفسُها **تَعَضُّ المؤقّتَ أشدّ**.
...
THE_BOUND_EVIDENCE = (
    "نصيبُ المرفوضات من الاقتراحات: ١٬١١٦ من ٣٬٤٢٤ في ذراع الأبديّ "
    "(٠٫٣٢٥٩)، و٥٬٩٧٠ من ٨٬٢٥٤ في ذراع المؤقّت (٠٫٧٢٣٣) — وبينهما "
    "٠٫٣٩٧٤، فالحدُّ الواحدُ في قيمته مختلفٌ في أثره"
)


def test_the_declared_depth_bites_the_two_arms_unequally() -> None:
    """الحدُّ واحدٌ في قيمته، **مختلفٌ في أثره** — العطلُ السابعَ عشر مقيسًا."""

    text = LOG.read_text(encoding="utf-8")
    stops = re.findall(r"الوقوف: التزاماتٌ \d+ \| اقتراحاتٌ (\d+) \| مرفوضاتٌ (\d+)", text)
    assert stops == [("3424", "1116"), ("8254", "5970")]
    shares = [int(refused) / int(offered) for offered, refused in stops]
    assert abs(shares[0] - 0.3259) < 5e-5
    assert abs(shares[1] - 0.7233) < 5e-5
    apart = abs(shares[0] - shares[1])
    assert abs(apart - 0.3974) < 5e-5
    assert THE_BOUND_BINDS_BOTH_ARMS is (apart < 0.01)
    assert "والعمقُ 24 لكليهما" in text  # القيمةُ واحدة، والأثرُ ليس واحدًا
