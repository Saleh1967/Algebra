...
FIXED = REPOSITORY / "deposits" / "arabic_token_run.log"
...
def render() -> str:
    ...
    add("")
    later = FIXED.read_text(encoding="utf-8")
    rows = (
        ("الألفاظ", r"عبرَ العدّادات (\d+) \| عبرَ الأسطر \d+", True),
        ("الهياكلُ المتمايزة", r"هياكلُ متمايزة: (\d+)", True),
        ("نصيبُ الثلاث", r"نصيبُ الثلاث: \d+ من \d+ = ([\d.]+)", False),
        ("نصيبُ «بلا علامة»", r"بلا علامة\) \d+ \| نصيبٌ ([\d.]+)", False),
        ("H(الحال)", r"H\(الحال\) = ([\d.]+)", False),
        ("أزواجُ ماركوف", r"أزواجٌ (\d+)", True),
        ("I(الجار)", r"\n  I = ([\d.]+)", False),
        ("I(حرفُ الخاتمة)", r"I\(الحال؛ حرفُ الخاتمة\) = ([\d.]+)", False),
        ("حروفٌ خاليةُ الحال", r"لا تحمل حالًا ألبتّة: (\d+)", True),
        ("نصيبُ ثابتِ الحال", r"نصيبُ الثابت من الهياكل: ([\d.]+)", False),
        ("نصيبُ كتلةِ الثابت", r"نصيبُ كتلة الثابت من الألفاظ: ([\d.]+)", False),
        ("انتقالُ الثبات", r"باقٍ ثابتًا في الفرديّ \d+ \| نصيبٌ ([\d.]+)", False),
    )
    add("## الحدُّ المُصحَّح — ومقابلةُ السجلّين")
    add("")
    add("**وقد سقط الشرطُ التاسعُ ففتح خللًا في الختم نفسِه**: حدُّ اللفظ")
    add("«أطولُ متتاليةٍ بلا فراغ» يبتلع وسمَ الترقيم. **فأُعيد الختمُ**")
    add("(`494465d1…`) بحدٍّ مُصحَّح — اللفظُ ما فيه محرفٌ عربيٌّ واحدٌ على")
    add("الأقلّ — **وصمدت شروطُه الاثنا عشرَ كلُّها**.")
    add("")
    add("| | بالحدّ المعطوب | بالحدّ المُصحَّح |")
    add("|---|---|---|")
    for title, pattern, whole in rows:
        (before,) = grab(pattern, text)
        (after,) = grab(pattern, later)
        pair: tuple[str, str] = (
            (grouped(int(before)), grouped(int(after)))
            if whole
            else (eastern(before), eastern(after))
        )
        add(f"| **{title}** | {pair[0]} | {pair[1]} |")
    add("")
    add("**ولا نتيجةَ انقلبت**: الخللُ مسَّ **٥٫٢٪ من الألفاظ** ولم يقلب")
    add("حكمًا واحدًا سوى الشرط الذي كشفه. **وهذا لا يُبرّئه** — يُقال إنّه")
    add("لم يقلب، لا إنّه لم يكن.")
    add("")
    add("**وأشدُّ ما فيه أنّه يُرى في الكتلة ولا يكاد يُرى في العدد**: نصيبُ")
    add("ثابتِ الحال من الهياكل لم يتحرّك (٠٫٥٧٧١ ⟶ ٠٫٥٧٦٩)، **وكتلتُه**")
    add("تحرّكت (٠٫٣٩٦٤ ⟶ ٠٫٣٦٣٣) — لأنّ الوسمَ كان **هيكلًا واحدًا ثابتَ")
    add("الحال يحمل ٤٬٢٨٧ لفظًا**.")
    add("")
    add("**وخانةُ الحال تخلو عند موضعٍ واحدٍ لا غير** بعد التصحيح: حرفٌ")
    add("واحدٌ بـ**٢٬٥٩٢** وقوعًا لا تُرى عليه علامةٌ في المجمَّد كلِّه.")
    add("**وخلوُّه مُصنَّفٌ لا مُصفَّر.**")
    add("")
